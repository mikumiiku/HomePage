...
with sync_playwright() as p:
 ...
 def open_settings():
  page.locator('[aria-controls="game-settings"]').click();expect(page.locator('#game-settings')).to_be_visible()
 def close_settings():
  page.locator('#game-settings header button').click();expect(page.locator('#game-settings')).not_to_be_visible()
 ...
 open_settings();page.locator('#chess-history button').first.click();expect(page.locator('#chess-status')).to_contain_text('查看');page.locator('#chess-live').click()
 # Cancel confirmation and ensure game survives.
 open_settings();page.locator('#chess-new').click();expect(page.locator('#chess-confirm')).to_be_visible();page.keyboard.press('Escape');expect(page.locator('#chess-history button')).to_have_count(2)
 with page.expect_download() as download:open_settings();page.locator('#chess-export').click()
 ...
 seed(fixture());open_settings();page.locator('#chess-resign').click();page.locator('#chess-accept').click();expect(page.locator('#chess-status')).to_have_text('黑方获胜');page.reload();expect(page.locator('#chess-status')).to_have_text('黑方获胜')
 ...
 for bad in ['{bad-json',dict(v=99,t=1,d=fixture()['d'])]:
  seed(bad);open_settings();expect(page.locator('#chess-backup')).to_be_visible();original=page.evaluate('(key)=>localStorage.getItem(key)',KEY);close_settings();move('e2','e4');page.wait_for_function('document.querySelectorAll("#chess-history button").length===2');assert page.evaluate('(key)=>localStorage.getItem(key)',KEY)==original
 ...
 for width,height in [(1440,1000),(1024,768),(768,1024),(390,844),(320,740),(844,390)]:
  ...
  page.wait_for_timeout(160)  # fit() 的 ResizeObserver 需要一两帧重算，立即断言会撞上过渡态
 ...
 open_settings();page.locator('#chess-mode').click();page.screenshot(path=str(OUT/'select-open.png'));page.keyboard.press('Escape')
